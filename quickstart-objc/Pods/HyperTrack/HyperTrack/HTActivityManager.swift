...
class HTActivityManager: NSObject {
    ...
    func saveActivityChanged(activity:CMMotionActivity) {
        ...
        DDLogInfo("Saving activity changed event" + currentActivity)
        DDLogInfo("Confidence : " + getActivityConfidence(activity: activity).description)
    ...
    func getOldActivity()-> HTActivity? {
        if let oldActivity = Settings.getActivity() {
            ...
            if let location = Settings.getActivityLocation(){
                ...
                return HTActivity.init(confidence: confidence!, startDate: date!, activityType: activityType,location: nil)
            }
        }
        return nil
    }
